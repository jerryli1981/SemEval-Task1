...
if args.load == 'true' then
  ...
  model = LSTMSim_ent.load(model_save_pre_path)
else
...
for i = 1, num_epochs do
  ...
  local dev_ent_score = accuracy(dev_predictions, dev_dataset.ent_labels)
...
local test_score = accuracy(test_predictions, test_dataset.ent_labels)
